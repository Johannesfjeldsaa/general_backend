# === === === === === === === === === === === === === === === === === ===
#
# Description: This module provides utility functions for saving
# figures, xarray datasets, and information to files in a
# safe and structured way.
#
# It includes functions to handle overwriting filepaths and variables
# in xarray datasets, ensuring that existing files or variables
# are not unintentionally overwritten unless explicitly allowed.
# It also provides functionality to save figures and datasets
# in specified formats (e.g., NetCDF for xarray datasets, YAML for
# information).
#
...
def validate_file(
    ...
    if not file_path.suffix in expected_suffix:
...
def check_filepath(
    ...
    -------
    bool
        True if the file can be saved (either it doesn't exist or overwrite is allowed),
        False if the file already exists and overwrite is not allowed.
    """

    file_path = Path(file_path).resolve()

    # default write to True, if the file exists, we will check if we can overwrite it
    write = True
    if file_path.exists():
    ...
    if write and not file_path.parent.exists():
        ...
        logger.info(f"Created directory: {file_path.parent} to save {file_path.name}")
...
def check_variable_overwrite(
    ...
    -------
    bool
        True if the variable can be saved (either it doesn't exist or overwrite is allowed),
        False if the variable already exists and overwrite is not allowed.
    """
...
def overwrite_handler(
    ...
    if overwrite is True:
        ...
        err_msg = f"Invalid value for overwrite: {overwrite}. Expected True, False, or 'prompt'."
...
def save_xarray_to_netcdf(
    ...
    if check_filepath(file_path, overwrite):
        ...
        logger.info(f"NetCDF dataset saved to {file_path}")
        return True
    return False